...
const publisherFromConfig = (bucketName: string) => {
...
describe('AwsS3Publish', () => {
  ...
  describe('getReadiness', () => {
    it('should validate correct config', async () => {
      const publisher = publisherFromConfig('bucketName');
    ...
    it('should reject incorrect config', async () => {
      const publisher = publisherFromConfig('errorBucket');
      expect(await publisher.getReadiness()).toEqual({
        isAvailable: false,
      });
    });
  ...
  describe('publish', () => {
    ...
    it('should fail to publish a directory', async () => {
      ...
      await expect(fails).rejects.toMatchObject({
        message: `Unable to upload file(s) to AWS S3. Error: Failed to read template directory: ENOENT, no such file or directory '${wrongPathToGeneratedDirectory}'`,
      });
    });
  });
  ...
  describe('fetchTechDocsMetadata', () => {
    ...
    it('should return an error if the techdocs_metadata.json file is not present', async () => {
      ...
      const fails = publisher.fetchTechDocsMetadata(entityName);
